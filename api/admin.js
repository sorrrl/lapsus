...
app.get("/addresources", async (req, res) => {
    ...
    app.get("/api/admin/users/search", async (req, res) => {
        ...
        if (!req.session.pterodactyl) return res.json({status: "unauthorized"});
        ...
        if (await cacheaccount.statusText == "Not Found") return res.json({status: "unauthorized"});
        let cacheaccountinfo = JSON.parse(await cacheaccount.text());
    
        req.session.pterodactyl = cacheaccountinfo.attributes;
        if (cacheaccountinfo.attributes.root_admin !== true) return res.json({status: "unauthorized"});
        
        if (!req.query.id) return res.json({status: "error", message: "Missing user ID"});

        let userId = req.query.id;
        let pterodactylId = await db.get("users-" + userId);
        
        if (!pterodactylId) return res.json({status: "error", message: "User not found"});
        ...
        let coins = newsettings.api.client.coins.enabled == true ? (await db.get("coins-" + userId) ? await db.get("coins-" + userId) : 0) : 0;
        let extra = await db.get("extra-" + userId) ? await db.get("extra-" + userId) : {
            ram: 0,
            disk: 0,
            cpu: 0,
            servers: 0
        ...
        if (await userinforeq.statusText !== "Not Found") {
            let userinfo = await userinforeq.json();
            username = userinfo.attributes.username || "Unknown";
        }
    ...
    app.get("/api/admin/stats", async (req, res) => {
        ...
        // Get total users from Pterodactyl
        let usersReq = await fetch(
            newsettings.pterodactyl.domain + "/api/application/users",
            {
                method: "get",
                headers: { 'Content-Type': 'application/json', "Authorization": `Bearer ${newsettings.pterodactyl.key}` }
            }
        ...
        let totalUsers = usersData.meta?.pagination?.total || 0;

        // Get total servers from Pterodactyl
        let serversReq = await fetch(
        ...
        let serversData = await serversReq.json();
        let totalServersCount = serversData.meta?.pagination?.total || 0;
        ...
        if (serversData.data && Array.isArray(serversData.data)) {
            for (let server of serversData.data) {
                if (server.attributes) {
                    totalRAM += server.attributes.limits?.memory || 0;
                    totalDisk += server.attributes.limits?.disk || 0;
                    totalCPU += server.attributes.limits?.cpu || 0;
                }
            }
        }

        // Get total coins (simplified - would need to iterate all users)
        // For now, we'll use a placeholder or calculate from known users
        let totalCoins = 0;
        // Note: To get accurate total coins, we would need to iterate all Discord users
        // This is a simplified version that shows the structure

        // Get active coupons count (simplified)
        let activeCoupons = 0;
        // Note: To get accurate coupon count, we would need access to all coupon keys
